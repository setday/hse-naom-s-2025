...
#include "lse_solvers/gaussian_elimination_pivoting.h"
...
public:
   ...
   */
  void set_initial_RHS( double* rhs )
  { // utilize initial condition @t=T
    for ( int i = 1; i <= m - 1; i++ )
    {
      for ( int j = 1; j <= n - 1; j++ )
      {
    ...
    update_RHS_boundary(
        rhs, 0.0 ); // utilize boundary condition @tau=0 (option is expired)
  }
  ...
  void update_RHS_boundary( double* rhs, double current_tau ) const
  {
    ...
    // double const8 = m_oe->K * std::exp((m_oe->d - m_oe->r) * current_tau) /
    // m_oe->h_s_max;
    ...
    for ( int j = 0; j <= n; j++ )
    {
      ...
      rhs[oj] = std::max( const9 * j - const10,
                          0.0 ); // relu(e^(-d * tau) * S - e^(-r * tau) * K)
  ...
  void operator()( double current_time, const double* current_state, double* rhs ) const override
  {
    ...
    for ( int i = 1; i <= m - 1; i++ )
    {
      ...
      for ( int j = 1; j <= n - 1; j++ )
      {
        ...
        rhs[ij] -= temp_const * ( current_state[ijp] - 2 * current_state[ij] +
                                  current_state[ijm] );
        ...
        rhs[ij] -= temp_const * ( current_state[ipj] - 2 * current_state[ij] +
                                  current_state[imj] );

        temp_const = const_dv_ds_i * std::pow( j, m_oe->beta );
        rhs[ij] -= temp_const * ( current_state[ipjp] - current_state[ipjm] -
                                  current_state[imjp] + current_state[imjm] );
      }
    }
...
private:
  ...
  void construct_SLAE()
  {
    ...
    double const_ds_ds = m_oe->tau * m_oe->sigma * m_oe->sigma / 2 *
                         m_oe->h_v_max *
                         std::pow( m_oe->h_s_max, 2 * m_oe->beta - 2 );
    double const_dv_dv =
...
private:
   ...
   */
  [[nodiscard]] int get_Q_index( int v, int s ) const
  {
    return v * ( n + 1 ) + s;
  }
};
